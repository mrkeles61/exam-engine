...
function MatchBadge({ name, confidencePct }: { name: string | null; confidencePct: number }) {
  ...
  return (
    <div
      ...
      title={t('workspace.templateMatchTooltip')}
    >
...
function ConfidenceStack({ segmentation, ocr, grading }: {
  ...
  return (
    <div className="flex flex-col gap-1 min-w-[120px]">
      <ConfBar label={t('workspace.confidence.segmentation')} value={segmentation ?? 0} />
...
function SubStrip({
  ...
  return (
    <div className="bg-white border-b border-slate-200">
      <div className="mx-auto max-w-[1600px] h-14 px-5 flex items-center gap-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {isApproved ? (
            <button onClick={onReopen}
              className="h-9 px-4 rounded-lg bg-slate-100 text-slate-700 text-xs font-semibold
                         hover:bg-slate-200 flex items-center gap-1.5 border border-slate-200">
              <span className="material-symbols-outlined text-base">lock_open</span>
              {t('common.reopen')}
            </button>
          ) : (
            <button onClick={onApprove}
              className="h-9 px-4 rounded-lg bg-iku-red text-white text-xs font-semibold
                         hover:bg-iku-red-dark flex items-center gap-1.5 shadow-sm">
...
function QuestionListWithEditablePoints({
  ...
  totalScore, totalMaxPoints, grade, pendingScores,
}: {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[10px] text-iku-red font-semibold cursor-pointer hover:underline">{t('workspace.fullScreen')}</span>
      </div>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <section className="col-span-12 lg:col-span-3">
            <QuestionListWithEditablePoints
              ...
              totalScore={(result.mc_score ?? 0) + (result.open_score ?? 0)}
              totalMaxPoints={(result.mc_total ?? 0) + (result.open_total ?? 0)}
              grade={result.grade}
            />
          </section>
