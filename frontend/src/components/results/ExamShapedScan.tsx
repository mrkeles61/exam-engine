...
function InitialsAvatar({ name, size }: { name: string; size: number }) {
  ...
  return (
    <div
      style={{ width: size, height: size, background: `hsl(${hue} 55% 60%)` }}
...
function MCBlock({ answer, thumbnail }: { answer: AnswerDetail; thumbnail?: boolean }) {
  ...
  return (
    <div className="absolute inset-0 px-2 py-1 overflow-hidden font-sans text-slate-900"
      ...
      {/* ReservedSpace intentionally removed — no ruled-line padding after any question */}
    </div>
  )
}
...
export default function ExamShapedScan({
  ...
  studentNumber, studentName, thumbnail = false,
}: Props) {
  const { t } = useLang()
  ...
  return (
    <div
      className={`relative bg-white w-full ${thumbnail ? 'border border-slate-300' : 'shadow-lg border border-slate-200 rounded-sm'}`}
        ...
        >
          ...
          <div className={`flex items-end justify-between ${thumbnail ? 'mt-0.5' : 'mt-1.5'}`}>
            ...
            <div className="flex items-center gap-1.5">
              <InitialsAvatar name={studentName ?? ''} size={thumbnail ? 14 : 28} />
              <div className="leading-tight text-right">
                <p className={`font-semibold text-slate-800 ${thumbnail ? 'text-[5px]' : 'text-[9px]'}`}>
                  {studentName ?? '—'}
                </p>
                <p className={`text-slate-500 ${thumbnail ? 'text-[3px]' : 'text-[6px]'}`}>
                  {t('scan.signature')}: ________
                </p>
              </div>
            </div>
          </div>
